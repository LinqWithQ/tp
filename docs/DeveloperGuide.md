...
2.  Should support an effectively unbounded number of contacts, limited only by the user’s local storage capacity.
3.  Should detect likely duplicate entries (e.g., same phone number and/or same email, or same name with matching contact info) and warn the user before saving, to reduce accidental data errors.
4.  All user data should be stored locally on the user’s device.
